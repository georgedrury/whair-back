...
exports.signup = async (req, res) => {
    ...
    if (userExists)
        return res.status(403).json({
            error: 'Email already seems to be signed up'
        }); 
...
exports.signin = (req, res) => {
    ...
    User.findOne({email}, (err, user) => {
        ...
        // create authenticate in method model and use here
...
exports.requireSignin = expressJwt({
    // If the token is valid then expres-jwt opens the verified users id 
...
exports.forgotPassword = (req, res) => {
    ...
    User.findOne({ email }, (err, user) => {
        ...
        const token = jwt.sign(
            { _id: user._id, iss: "WHAIRBACK" },
            process.env.JWT_SECRET
        );
 
        // email data
        const emailData = {
            from: "whairdev@gmail.com",
            to: email,
            subject: "Whair: Password reset instructions",
